...
data = json.load(open(os.path.join(PROJ, "shortcuts.json"), encoding="utf-8"))   # encoding 명시 — Windows(cp949 로케일)에서도 동작
...
tpl = open(os.path.join(PROJ, "viewer.template.html"), encoding="utf-8").read()
open(os.path.join(PROJ, "viewer.html"), "w", encoding="utf-8").write(tpl.replace("/*__DATA__*/", json.dumps(data, ensure_ascii=False)))
