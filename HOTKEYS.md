...
## CapsLock 창 드래그 (이동·리사이즈)

**AltDrag·frogcontrol 스타일** — 모달 호출 없이, `CapsLock`을 누른 채로 마우스로 창을 드래그합니다. 커서 아래 창을 **창 안 아무 지점에서나** 잡습니다(타이틀바 불필요):

- `CapsLock + 왼쪽클릭 드래그` → 창 **이동**
- `CapsLock + 오른쪽클릭 드래그` → 창 **리사이즈** — 클릭 지점에서 가장 가까운 모서리/변이 축별로 독립적으로 따라옵니다(오른쪽 절반을 잡으면 오른쪽 변이, 왼쪽 절반을 잡으면 왼쪽 변이 움직이는 식. 상/하도 동일). 최소 크기 120×80.

**켜는 법**: 메뉴바 아이콘 ▸ **`🪟 CapsLock 창 드래그 (이동·리사이즈)`** 체크마크 토글(꺼짐이 기본) — 켤 때 손쉬운 사용 권한이 필요하면 프롬프트가 뜹니다. 또는 `hotkeys.json`에 직접:
```json
"window_drag": {"enabled": true, "modifier": "caps"}
```

> **⚠️ Karabiner를 쓴다면**: Karabiner-Elements에 "맨 CapsLock" 리맵 규칙이 있으면 CapsLock을 누르고 뗄 때의 진짜 이벤트가 우리 앱에 안 올 수 있습니다(수식키+Caps 조합만 통과). 이 환경에서는 `"modifier": "fn"`으로 바꾸세요 — `🌐(fn)` 키를 누른 채 드래그하는 방식으로 동작하며, 메뉴 토글이 아니라 config로만 설정합니다.

이건 **창 관리 중 핵심 기능**이고, 거의 모든 다른 창 매니저(Rectangle·Magnet)는 단축키로만 지원하므로 마우스 기반은 여기서만 됩니다.

## 로드맵 — 다음 단계

우리가 다음에 구현할 기능들입니다. 사용해보고 피드백을 보내주세요.

### 1️⃣ 키보드만으로 창 이동·리사이즈·스냅
- 좌/우/위/아래 모니터 절반 맞춤 · 모서리·중앙 위치 고정 · 창 크기 조절
- 예: `⌃⌥←` → 왼쪽 절반 / `⌃⌥↑` → 위쪽 절반 등

### 2️⃣ 뷰어 그리드에 레이어별 충돌 하이라이트
