...
# ── 공유 기본값 팩: win/defaults/<app>/<version>.json (tracked — 맥 defaults/의 Windows판) ──
# 앱 "기본값"만(PII 없음). 덤프/키맵 파일이 있는 기기가 SAVE → 커밋 → 다른 기기는 SEED.
# 맥 defaults/와 분리하는 이유: 같은 앱이라도 mac/win 조합이 달라 서로 seed하면 안 됨.
WIN_DEFAULTS = os.path.join(PROJ, "defaults")

def _verkey(v): return tuple(int(x) for x in re.findall(r"\d+", v or ""))

def save_win_pack(app, version, scope, ents):
    if not ents or os.name != "nt":   # mac에서의 테스트 실행이 mac 조합을 win corpus에 오염시키지 않게
        return
    d = os.path.join(WIN_DEFAULTS, app); os.makedirs(d, exist_ok=True)
    path = os.path.join(d, str(version) + ".json")
    new = json.dumps({"app": app, "version": str(version), "scope": scope, "entries": ents},
                     ensure_ascii=False, indent=1, sort_keys=True)
    if os.path.exists(path) and open(path, encoding="utf-8").read() == new:
        return   # unchanged → git churn 없음
    open(path, "w", encoding="utf-8").write(new)
    print(f"  → win/defaults/{app}/{version}.json 저장 (커밋하면 다른 기기에 공유)")

def newest_win_pack(app):
    best = None
    for path in glob.glob(os.path.join(WIN_DEFAULTS, app, "*.json")):
        try:
            data = json.load(open(path, encoding="utf-8"))
        except Exception:
            continue
        if not best or _verkey(data.get("version")) > _verkey(best.get("version")):
            best = data
    return best

def seed_win_pack(app):   # 로컬 덤프/키맵이 없을 때 tracked 팩에서 seed. 성공 시 True
    data = newest_win_pack(app)
    if not data:
        return False
    for e in data.get("entries", []):
        e2 = dict(e); e2["detail"] = (e2.get("detail", "") + " · seed").strip(" ·")
        entries.append(e2)
    print(f"  {app}: 로컬 덤프 없음 → win/defaults 팩 {data.get('version')} seed ({len(data.get('entries', []))}개)")
    return True

def collect_win_packs():   # 아직 아무 소스도 없는 앱을 팩에서 seed (맥 collect_community 대응)
    have = {e["scope"] for e in entries}
    for d in glob.glob(os.path.join(WIN_DEFAULTS, "*")):
        if not os.path.isdir(d):
            continue
        data = newest_win_pack(os.path.basename(d))
        if data and data.get("scope") not in have:
            seed_win_pack(data["app"])

# ── 공식 문서 팩: win/packs/*.json (tracked — 맥 web_shortcuts.json 대응) ────
# 형식: {"app":"Excel","ref":"<공식 문서 URL>","shortcuts":[["Ctrl+N","동작","섹션(선택)"],…]}
# 리본/메뉴가 API로 안 보이는 모던 앱(Office·카톡·한글…)의 커버리지를 제조사 공식 문서로 채운다.
WIN_PACKS = os.path.join(PROJ, "packs")

def collect_doc_packs():
    import html
    for path in sorted(glob.glob(os.path.join(WIN_PACKS, "*.json"))):
        try:
            d = json.load(open(path, encoding="utf-8"))
        except Exception as e:
            print("  pack parse fail", os.path.basename(path), e); continue
        app = d.get("app")
        if not app or d.get("error"):
            continue
        n = bad = 0
        for row in d.get("shortcuts") or []:
            ...
            add(pk[0], pk[1], html.unescape(str(row[1])), source="web", scope=app,
                detail=(ctx + " · " if ctx else "") + "공식 문서", group=app); n += 1
        print(f"  {app} 문서팩 {n}개" + (f" (파싱 불가 {bad})" if bad else "") + f" — {os.path.basename(path)}")

def collect_adobe_dumps():   # Adobe 전체 키맵 추출기(repo 루트 dump_*.py) 실행 → win/defaults 팩 생성 → collect_win_packs가 seed
    if os.name != "nt":
        return
    import subprocess
    for s in ("dump_ae.py", "dump_premiere.py", "dump_photoshop.py", "dump_illustrator.py"):
        try:
            r = subprocess.run([sys.executable, os.path.join(ROOT, s)], capture_output=True, text=True, timeout=180)
            out = ((r.stdout or "") + (r.stderr or "")).strip()
            if r.returncode == 0 and out:
                print("  " + out.replace("\n", "\n  "))   # 성공 — 미매핑 키 경고 등 전체 출력 (붙여주면 채움)
            elif "못 찾음" not in out:                      # 미설치는 조용히 스킵
                print(f"  {s}: {out.splitlines()[-1] if out else '실패'}")
        except Exception as e:
            print(f"  {s} 실행 실패: {e}")

def vscode_version():   # 설치된 VS Code 버전 — resources/app/package.json
    for c in (r"%LOCALAPPDATA%\Programs\Microsoft VS Code\resources\app\package.json",
              r"%ProgramFiles%\Microsoft VS Code\resources\app\package.json"):
        c = os.path.expandvars(c)
        try:
            return json.load(open(c, encoding="utf-8")).get("version") or "0"
        except Exception:
            pass
    return "0"
...
#           → 내용 전체를 repo 루트에 vscode_default_keybindings.json 으로 저장 (gitignored 중간 산물 —
#           build가 파싱해 win/defaults/VS Code/<ver>.json 팩(tracked)으로 승격한다).
def collect_vscode():
    cands = [(os.path.join(ROOT, "vscode_default_keybindings.json"), "기본"),
             (os.path.expandvars(r"%APPDATA%\Code\User\keybindings.json"), "사용자 지정")]
    for p, kind in cands:
        if not os.path.exists(p):
            if kind == "기본" and not seed_win_pack("VS Code"):   # 덤프 없으면 공유 팩에서 seed
        ...
        if kind == "기본" and n:
            save_win_pack("VS Code", vscode_version(), "Code", entries[start:])   # 기본값만 팩으로 승격(사용자 지정 제외)
