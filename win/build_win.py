...
def collect_win32_menus():
    ...
    def exe_of(hwnd):   # 창 → 프로세스 exe 이름 (스코프)
        pid = wintypes.DWORD()
        ...
        return os.path.splitext(os.path.basename(buf.value))[0] if ok else None

    stats = {}
    def walk(hmenu, path, scope, depth=0):
        if depth > 6:
            return
        for i in range(max(0, user32.GetMenuItemCount(hmenu))):
            ...
            if sub:
                walk(sub, path + ([label] if label else []), scope, depth + 1)
            elif "\t" in label:
                name, acc = label.split("\t", 1)
                pk = parse_accel(acc)
                if pk:
                    add(pk[0], pk[1], " ▸ ".join(path + [name.strip()]), source="app menu",
                        scope=scope, detail="Win32 메뉴 스캔", group=scope)
    ...
    def cb(hwnd, _):
        try:
            ...
            if not hm:
                return True
            scope = exe_of(hwnd) or "?"
            if (scope, hm) in seen:   # 같은 앱의 창 여러 개 → 메뉴 한 번만
                return True
            seen.add((scope, hm))
            walk(hm, [], scope)
        except Exception:
