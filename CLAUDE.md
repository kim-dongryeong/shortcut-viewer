...
- `win/` — Windows 대응(수집기는 스켈레톤). `build_win.py`가 같은 스키마로 수집(Win→`cmd`·Alt→`opt` 토큰 매핑)해 `meta.platform:"windows"`를 심으면, 공유 뷰어가 **자동으로 Windows 키보드 표기**로 전환(`IS_WIN` in template): ⊞Win/Alt/Ctrl 키캡, 104키 하단행(☰메뉴), Win 넘패드(NumLock·세로 +/Enter), `Ctrl+Shift+S` 표기, win-full/tkl/노트북 프리셋, 흔한키=Ctrl 기반. mac 데이터(platform 없음)는 영향 없음.
