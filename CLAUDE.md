...
- `packaging/` — 일반 사용자 배포. `packaging/mac/build_dmg.sh`는 axmenudump 컴파일+`build.py`+corpus(`defaults/`+`web_shortcuts.json`)를 `Shortcut Viewer.app`으로 번들(ad-hoc 서명)해 `.dmg`로 — repo clone·터미널 불필요, 더블클릭이면 corpus 수천 개가 바로 보임. `packaging/win/build_exe.bat`은 같은 개념을 PyInstaller onefile `ShortcutViewer.exe`로(Windows에서만 빌드 가능 — 크로스 컴파일 불가). 둘 다 서명 없음(v1, 무료) → Gatekeeper/SmartScreen 경고 1회. 빌드 산출물(`packaging/*/dist`, `*.exe`)은 gitignore, 스크립트만 tracked.
- `win/` — Windows 대응. `build_win.py`가 같은 스키마로 수집(Win→`cmd`·Alt→`opt` 토큰 매핑): 시스템+탐색기 시드 55 · **실행 중 앱 메뉴 스캔**(axmenudump 대응 — ctypes `GetMenu`/`GetMenuStringW`, 라벨 `\t` 뒤 accelerator 파싱; 클래식 Win32 메뉴 앱만, Chrome/Office/UWP 불가) · AutoHotKey(`^!+#` 핫키) · PowerToys Keyboard Manager(VK 코드 `;`구분 디코드) · **.lnk 바로 가기 키**([MS-SHLLINK] 헤더 0x40 HotKey, 시작메뉴+바탕화면 walk) · Windows Terminal settings.json(JSONC; 기본 키는 패키지 내부라 사용자 지정만) · VS Code(기본 덤프+사용자 keybindings, chord→`cmods/ckey`). 수집기마다 0개 사유를 콘솔에 진단 출력. **공유 corpus 2종(둘 다 tracked·PII 없음)**: `win/defaults/<app>/<ver>.json` = 앱 기본값 팩(VS Code 덤프 승격 + Adobe 추출기 산출 — mac `defaults/`와 분리, 플랫폼 조합이 다르므로 절대 혼용 금지) — 로컬 덤프 없는 기기는 최신 팩에서 seed; `win/packs/*.json` = 공식 문서 팩(리본/커스텀 UI라 스캔 불가한 Office·카톡·한글·Acrobat 등, 형식 `{"app","ref","shortcuts":[["Ctrl+N","동작","섹션?"]]}`, `parse_keystr`로 파싱, source `web`). **Adobe 추출기 4종(dump_ae/premiere/photoshop/illustrator)은 플랫폼 인식**: Windows 경로 글롭 + Win 수식키 토큰/마스크(AI: win은 64=Ctrl) + 출력 `OUTBASE`(nt→win/defaults) — build_win이 자동 실행(`collect_adobe_dumps`). `meta.platform:"windows"`를 심으면, 공유 뷰어가 **자동으로 Windows 키보드 표기**로 전환(`IS_WIN` in template): ⊞Win/Alt/Ctrl 키캡, 104키 하단행(☰메뉴), Win 넘패드(NumLock·세로 +/Enter), `Ctrl+Shift+S` 표기, win-full/tkl/노트북 프리셋, 흔한키=Ctrl 기반. mac 데이터(platform 없음)는 영향 없음.
